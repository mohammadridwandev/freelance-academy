      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {instructorData.map((mentor) => (
            ...
            >
              {/* Minimal Icon Square */}
              <div className="size-12 bg-teal-50 rounded-lg flex items-center justify-center text-teal-600 shrink-0 group-hover:bg-teal-600 group-hover:text-white transition-colors duration-300">
                <User2 size={22} />
              </div>

              {/* Identity - Strictly text-base for name */}
              <div className="flex space-y-1 flex-col min-w-0">
                <span className="text-[13px] font-bold text-teal-600 uppercase ">
                  {mentor.position}
                </span>
